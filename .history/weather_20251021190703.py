...
def find_min(weather_data):
    ...
    if not weather_data:
        return ()   
    # Handle case where list contains string representations of numbers
    weather_data = [float(i) for i in weather_data]     

    min_value = min(weather_data)
    # Find the last occurrence index of the minimum value
    position = len(weather_data) - 1 - weather_data[::-1].index(min_value)
    
    return (float(min_value), position)     
    # Check if the list is empty
    if not weather_data:
        return ()       
    # Convert all elements to float for accurate comparison
